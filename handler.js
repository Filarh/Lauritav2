...
/*numero mínimo de mensajes para no ser limpiado automáticamente
por el bot cuando se ejecute el comando de limpieza */
let zona_segura = 10;
/*==============::)))))Por Mist===================*/
        ...
        try {
            ...
            if (user) {
              ...
              if (!user.registered) {
                ...
                if (!isNumber(user.limit)) user.limit = 10
                if (!isNumber(user.lastclaim)) user.lastclaim = 0
                if (!isNumber(user.mcount)) user.mcount = 0
                if (!('callada' in user)) user.callada = false
                if (!('rName' in user)) user.rName = 'NoRegistrado'
                if (!('rAge' in user)) user.rAge = 'NoRegistrado'
                if (!('rCity' in user)) user.rCity = 'NoRegistrado'
                if (!('reinfo' in user)) user.rCity = 'Nada'
            } else
                global.db.data.users[m.sender] = {
                    exp: 0,
                    limit: 10,
            ...
            if (chat) {
                ...
                if (!('allowedN' in chat)) chat.allowedN = '500,502,503,504,505,506,507,508,509,51,52,53,54,55,56,57,58,590,591,592,593,594,594,595,596,597,598,809,829,849'
                if (!('International' in chat)) chat.International = true
            } else
                global.db.data.chats[m.chat] = {
                    isBanned: false,
                    welcome: true,
                    ...
                    antiArab: false,
                    expired: 0,
                    allowedN : '500,502,503,504,505,506,507,508,509,51,52,53,54,55,56,57,58,590,591,592,593,594,594,595,596,597,598,809,829,849',
                    International : true
                }
            ...
            if (settings) {
                ...
                if (!('zonasegura' in settings)) settings.zonasegura = zona_segura
            ...
            if (m.isGroup) {
                global.db.data.users[m.sender].mcount += 1
                console.log(global.db.data.users[m.sender].name + " tiene: " + global.db.data.users[m.sender].mcount + " mensajes.");
            }
        } catch (e) {
            console.error(e)
        }
...
if (action == 'add'){   
    let allowed = global.db.data.chats[id].allowedN
    ...
    console.log('\x1b[36m%s\x1b[0m', id);
   if (!global.db.data.chats[id].International && !checkIfStringStartsWith(user, listallowed)){
        conn.sendHydrated(id, `lo siento pero el prefijo de tu número no está permitido en este grupo 👋`, null, null, null, null, null, null, [['🌹OWNER🌹', `/owner`]])
        await delay(1000)
        conn.groupParticipantsUpdate(id, [user], 'remove')
   } 
    //conn.sendHydrated('593962888416@s.whatsapp.net', '💖¡Nuevo aterrizaje de ' + user +' en otro grupo!💖', null, null, null, null, null, null, [['🌹OWNER🌹', `/owner`]])
}else{
    try{
        global.db.data.robed[user].salio = true
    }catch{
        //conn.sendButton('593962888416@s.whatsapp.net', `${user} ocurrió un error al intentar darle atributo Verdadero a su salida, revisa la lista`,null, null, [[null, null]], false, null)
    }
